...
2. **补齐一个可靠的开发运行入口**
   - 至少保证新接手者能按文档完成 install + start
   - 修复 `pnpm start:all` 或提供等价的单命令方案

### P1

3. **把 Web 端主路径和持久化后的 API 再做一次联调**
...
5. **替换关键 AI mock 为真实可配置 provider 流程**
6. **验证并修复 Docker / 部署链路**
7. **继续补齐计划中的扩展能力（通知、更多后台能力、完整导出等）**
